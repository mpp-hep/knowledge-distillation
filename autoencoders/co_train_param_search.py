...
from hyper_models import HyperStudent
...
class HyperStudent(HyperStudent):
    def fit(self, hp, student_model, train_dataset, val_dataset,test_dataset, teacher_model, include_latent_loss, epochs, **kwargs):
        

        #model.fit(x, y, **kwargs)
        # define the model optimizers
        teacher_optimizer = tf.keras.optimizers.Adam(3e-3, amsgrad=True)
        student_optimizer = tf.keras.optimizers.Adam(3e-3, amsgrad=True)
        ...
        # teacher_model.save_weights(f'{output_dir}teacher_ckpt/teacher_ckpt')
        # student_model.save_weights(f'{output_dir}student_ckpt/student_ckpt')
           
        config = []
        for l in student_model.layers[3:-2]:
        ...
        if self.quantized:
            config = ['_Bits']
            for l in student_model.layers[3:-1]:
                if 'dense' not in l.name:
                    continue
                config.append('(')
            ...
            config = "_" + str(student_model.layers[-2].output.shape[1])
            modelname += config

        print(modelname)

        if not os.path.isdir(f'output/cotrain_results/{modelname}'):
            os.makedirs(f'output/cotrain_results/{modelname}')
        
        student_model.save(os.path.join(f'output/cotrain_results/{modelname}', f'student_{modelname}.h5'))
        teacher_model.save_weights(os.path.join(f'output/cotrain_results/{modelname}', f'teacher_model.h5'))
        
        teacher_model.load_weights("temp_t_weights.h5")

        return state_accumulator[-1][3]
...
def teacher_student_cotrain(data_file, 
    ...
    hstudent = HyperStudent(x_train.shape, 'mae',param_threshold=(3000,8000),quantized = True, expose_latent = True)
    ...
    teacher_model.save_weights("temp_t_weights.h5")

    tuner = keras_tuner.RandomSearch(
              hstudent,
              # objective='val_loss',
              max_trials=len(hstudent.model_configurations),
              overwrite=True,
              directory='output/hyper_tuning_cotrain',
              )
    ...
    test_dataset = tf.data.Dataset.from_tensor_slices((x_test, y_test)).batch(config['batch_size']).prefetch(tf.data.AUTOTUNE)
    ...
    tuner.search(
        ...
        epochs=1,
        batch_size=2048,
        include_latent_loss=include_latent_loss
        )
...
if __name__ == '__main__':
    ...
    parser.add_argument('--include-latent-loss', type=bool, default=False, help='include latent matching')
