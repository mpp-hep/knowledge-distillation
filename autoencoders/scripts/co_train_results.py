...
def main(args):
    ...
    with open(args.data_file, 'rb') as f:
        ...
        all_bsm_data, pt_scaler, _, _, _, _ = pickle.load(f)
    ...
    teacher_model.load_weights(f'{args.output_dir}teacher_ckpt/teacher_ckpt')
    student_model.load_weights(f'{args.output_dir}student_ckpt/student_ckpt')
    ...
    plt.legend(loc='best')
    plt.savefig(os.path.join(args.output_dir, f'loss_on_training.png'))
    ...
    history['val_loss'] = [0]
    # save results
    with h5py.File(f'{args.student_output_result}', 'w') as h5f:
    ...
    with h5py.File(f'{args.teacher_output_test_loss}', 'w') as h5f:
    ...
    with h5py.File(f'{args.teacher_output_signal_loss}', 'w') as h5f:
...
if __name__ == '__main__':
    ...
    parser.add_argument('--student-output-result', type=str,
        help='Student output file with results', required=True)
    parser.add_argument('--teacher-output-test-loss', type=str,
        help='Where is the data')
    parser.add_argument('--teacher-output-signal-loss', type=str,
        help='Where is the data')
