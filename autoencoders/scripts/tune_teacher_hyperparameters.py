...
def optimisation(args):
    ...
    print(f'x_val shape {datasets["x_val"].shape}, y_val shape is {datasets["y_val"].shape}')

    hypermodel = HyperTeacher(datasets['x_val'].shape)
    ...
    best_model.build(datasets['x_val'].shape[1:])
    best_model.summary()
    best_model.fit(
        x=datasets['x_train'],
        y=datasets['y_train'],
...
if __name__ == '__main__':
    ...
    parser.add_argument('teacher_loc', type=str,
        help='Path to save the best teacher')
