...
def plot_reconstruction(
    ...
    for i, feat in enumerate(input_featurenames):
        ...
        plt.savefig(f'output/plots/reco_{sample_name}_{feat}.pdf')
        plt.clf()
...
def main(args):
    ...
    with open(args.pt_scaler, 'rb') as f:
        pt_scaler = pickle.load(f)

    # load data
    datasets = np.load(args.dataset)

    background_loss, x_target, x_predicted  = evaluate(model, datasets['x_test'], pt_scaler)
    ...
    plt.savefig(f'output/plots/rocs.pdf')
    print('Saved your plots in output/plots/rocs.pdf')
    plt.clf()
    ...
    plt.legend(loc='best')
    plt.savefig('output/plots/bsm_loss.pdf')
    plt.clf()
